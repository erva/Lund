          ...
          if (moveToFirst()) {
            do {
              bankSms.add(
                  PlainSms(
                      threadId = getLong(
                          getColumnIndexOrThrow(Telephony.TextBasedSmsColumns.THREAD_ID)
                      ),
                      address = getString(
                          getColumnIndexOrThrow(Telephony.TextBasedSmsColumns.ADDRESS)
                      ),
                      date = getLong(getColumnIndexOrThrow(Telephony.TextBasedSmsColumns.DATE)),
                      dateSent = getLong(
                          getColumnIndexOrThrow(Telephony.TextBasedSmsColumns.DATE_SENT)
                      ),
