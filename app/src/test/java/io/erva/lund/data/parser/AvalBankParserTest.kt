...
class AvalBankParserTest {
  ...
  fun testWriteOff() {
    ...
    transaction?.let {
      ...
      Assert.assertEquals(
          it.parsedInfoDate, SimpleDateFormat("dd.MM.yyyy HH:mm").parse("13.07.2018 20:07")
      )
  ...
  fun testAnotherWriteOff() {
    ...
    transaction?.let {
      Assert.assertEquals(it.parsedCardNumber, "1234")
      Assert.assertEquals(
          it.parsedInfoDate, SimpleDateFormat("dd.MM.yyyy HH:mm").parse("13.07.2018 09:50:30")
      )
