...
class UkrSibBankParserTest {
  ...
  fun testWriteOff() {
    ...
    transaction?.let {
      ...
      Assert.assertEquals(
          it.parsedInfoDate, SimpleDateFormat("dd.MM.yyyy HH:mm").parse("29.04.2018 18:37")
      )
  ...
  fun testBlock() {
    ...
    transaction?.let {
      Assert.assertEquals(it.parsedCardNumber, "1234")
      Assert.assertEquals(
          it.parsedInfoDate, SimpleDateFormat("dd.MM.yyyy HH:mm").parse("29.04.2018 00:29")
      )
  ...
  fun testIncome() {
    ...
    transaction?.let {
      Assert.assertEquals(it.parsedCardNumber, "1234")
      Assert.assertEquals(
          it.parsedInfoDate, SimpleDateFormat("dd.MM.yyyy HH:mm").parse("18.04.2018 11:55")
      )
  ...
  fun testAnotherIncome() {
    ...
    transaction?.let {
      Assert.assertEquals(it.parsedCardNumber, "1234")
      Assert.assertEquals(
          it.parsedInfoDate, SimpleDateFormat("dd.MM.yyyy HH:mm").parse("17.04.2018 12:49:12")
      )
